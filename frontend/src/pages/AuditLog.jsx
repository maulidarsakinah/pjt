...
import { getAuditLogJourney, getAuditLogs } from "../services/api";
import "./AuditLog.css";
...
function getTodayInJakarta() {
  const parts = Object.fromEntries(
    LOG_DATE_FORMATTER.formatToParts(new Date())
...
const auditKpiDescriptions = {
  ...
  success:
    "Jumlah request yang berhasil diproses oleh server dengan status sukses.",
  failed:
    "Jumlah request yang gagal, termasuk error autentikasi atau response kode 4xx/5xx.",
...
function matchesFilters(log, filters, searchQuery = "") {
  const dateParts = Object.fromEntries(
    LOG_DATE_FORMATTER.formatToParts(new Date(log.time))
      .filter((part) => part.type !== "literal")
      .map((part) => [part.type, part.value]),
  );
...
function getLogKey(log) {
  return (
    log.id ||
    [log.trace_id, log.time, log.method, log.path, log.status_code, log.message]
      .map((value) => value ?? "")
      .join("|")
  );
}
...
function getJourneyLabel(entry) {
  const labels = {
    request_received: "Request diterima",
    authentication: "Bearer authentication",
    authorization: "Pemeriksaan permission",
    request_failed: "Request gagal",
    request_aborted: "Koneksi terputus sebelum response selesai",
    response_completed: "Response dikirim",
  };

  return (
    labels[entry.journey_stage] ||
    entry.event_action?.replaceAll("_", " ") ||
    entry.message ||
    "Aktivitas request"
  );
}
...
const AuditLogRow = memo(function AuditLogRow({ log, onSelect }) {
  ...
  return (
    <tr>
      ...
      <td>
        <span className={`audit-pill ${levelMeta.className}`}>
          {levelMeta.label}
        </span>
      </td>
      <td>
        <span className="audit-method-pill">{log.method || "-"}</span>
      </td>
      <td className="audit-endpoint" title={log.path || undefined}>
        {log.path || "-"}
      </td>
      <td>
      ...
      <td className="audit-latency">
        {Number.isFinite(log.latency_ms)
          ? `${Math.round(log.latency_ms)} ms`
          : "-"}
      </td>
      <td className="audit-trace" title={log.trace_id || undefined}>
        {log.trace_id || "-"}
      </td>
      <td>
  ...
  return SKELETON_ROWS.map((row) => (
    <tr className="audit-skeleton-row" key={row} aria-hidden="true">
      {Array.from({ length: 9 }, (_, column) => (
        <td key={`${row}-${column}`}>
          <span className="audit-skeleton-cell"></span>
        </td>
      ))}
    </tr>
  ));
...
const AuditKpiSection = memo(function AuditKpiSection({
  isDemoUser,
  metrics,
  total,
}) {
  ...
  const showInfoAfterIntent = useCallback(
    (event) => {
...
const AuditFilters = memo(function AuditFilters({
  appliedFilters,
  isLoading,
  onApply,
  onReset,
  ...
  return (
    <div className="audit-filter-panel">
      ...
      <div
        className={`audit-filter-content ${isOpen ? "is-open" : ""}`}
        ...
        <div className="audit-filter-grid">
          ...
          <div className="filter-group">
            ...
            <select
              ...
              onChange={(event) =>
                updateDraftFilter("method", event.target.value)
              }
            >
...
const AuditSearch = memo(function AuditSearch({ isLoading, onCommit }) {
  ...
  return (
    <label className="audit-search">
      <i
        className={`fa-solid ${isLoading ? "fa-hourglass-half" : "fa-magnifying-glass"}`}
      ></i>
      <input
        type="search"
        value={inputValue}
...
const AuditResults = memo(function AuditResults({
  error,
  isLoading,
  logs,
  onSelect,
      ...
      return (
        <ul className="audit-mobile-list" aria-busy="true">
          {SKELETON_ROWS.map((row) => (
            <li className="audit-mobile-skeleton" key={row}></li>
          ))}
        </ul>
      );
    }
  ...
  return (
    <div className="audit-table-scroll-shell">
      <div
        className="table-container audit-table-container"
        tabIndex="0"
        aria-label="Tabel audit log, dapat digeser horizontal"
      >
...
const AuditPagination = memo(function AuditPagination({
  ...
  const totalPages = isDemoUser
    ? 1
    : Math.max(1, Math.ceil(totalLogs / PAGE_SIZE));
      ...
      <div className="page-controls">
        ...
        {paginationItems.map((item) =>
          typeof item === "number" ? (
            ...
            <span className="audit-page-ellipsis" aria-hidden="true" key={item}>
              ...
            </span>
          ),
        )}
        <button
          ...
          disabled={
            isDemoUser || !hasMore || pageNumber >= totalPages || isLoading
          }
...
const AuditLog = () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) {
          const rows = Array.isArray(response.data)
            ? response.data.slice(0, PAGE_SIZE)
            : [];
          setLiveLogs(rows);
          setHasMore(Boolean(response.has_more));
          setTotalLogs(
            Number.isInteger(response.total) ? response.total : rows.length,
          );
        }
      } catch (requestError) {
  ...
  const filteredStaticLogs = useMemo(
    () =>
      STATIC_AUDIT_LOGS.filter((log) =>
        matchesFilters(log, appliedFilters, searchQuery),
      ),
    [appliedFilters, searchQuery],
  );
  const auditLogs = isDemoUser ? filteredStaticLogs : liveLogs;
  const metrics = useMemo(() => {
    const summary = auditLogs.reduce(
      (current, log) => ({
        ...
        latencyTotal:
          ...
          (Number.isFinite(log.latency_ms) ? log.latency_ms : 0),
        latencyCount:
          current.latencyCount + (Number.isFinite(log.latency_ms) ? 1 : 0),
      }),
      {
        successCount: 0,
        failedCount: 0,
    ...
    return {
      ...summary,
      avgLatency: summary.latencyCount
        ? summary.latencyTotal / summary.latencyCount
        : 0,
    };
  }, [auditLogs]);
  ...
  const loadJourney = async () => {
    if (!selectedLog?.trace_id || journeyLoading) return;

    setJourneyLoading(true);
    setJourneyError("");
    try {
      const response = await getAuditLogJourney(selectedLog.trace_id);
      setJourney(Array.isArray(response.data) ? response.data : []);
    } catch (requestError) {
      setJourneyError(
        requestError.message || "Gagal memuat perjalanan request.",
      );
    } finally {
  ...
  const exportLogs = () => {
    ...
    const csvRows = [
      ...
      ...auditLogs.map((log) =>
        columns.map((column) => escapeCsvValue(log[column])).join(","),
      ),
    ];
    ...
    <div className="view-section audit-log-page">
      <div className="header-section audit-log-header">
        <div>
          <h1>Audit Log</h1>
          <p>
            Riwayat request API, status autentikasi, dan trace aktivitas sistem.
          </p>
        </div>
        <button
        ...
        <div className="panel-header audit-table-header">
          <div>
            ...
            <div className="panel-subtitle">
              {isDemoUser
                ? "Menampilkan data statis untuk sesi demo."
                : "Data terbaru dari backend, diurutkan dari waktu terbaru."}
            </div>
          </div>
        </div>
      ...
      {selectedLog && (
        ...
        >
          ...
          >
            <div className="audit-detail-header">
              <div>
                ...
                <h3 id="audit-detail-title">
                  {selectedLog.message || "Aktivitas API"}
                </h3>
              </div>
              <button
            ...
            <div
              className={`audit-detail-visual ${selectedStatusMeta.className}`}
            >
            ...
            <div className="audit-detail-stack">
              <div className="audit-detail-item">
                <span>Status</span>
                <strong>
                  {selectedLog.status} / HTTP {selectedLog.status_code}
                </strong>
              </div>
              <div className="audit-detail-item">
                <span>Time</span>
              ...
              <div className="audit-detail-item">
                ...
                <strong>
                  {selectedLog.service || "-"} / {selectedLog.env || "-"}
                </strong>
              </div>
              <div className="audit-detail-item">
              ...
              <div className="audit-detail-item">
                ...
                <strong>
                  {selectedLog.user_name ||
                    (selectedLog.user_id ? "Pengguna" : "Publik")}
                  {selectedLog.user_id
                    ? ` · #${selectedLog.user_id}`
                    : " · Tanpa autentikasi"}
                </strong>
              ...
              {selectedLog.mutation && (
                <div className="audit-detail-item audit-detail-mutation">
                  <span>Perubahan Request</span>
                  <div className="audit-mutation-summary">
                    <strong>
                      Field:{" "}
                      {selectedLog.mutation.fields_touched?.join(", ") || "-"}
                    </strong>
                    <small>
                      Payload {selectedLog.mutation.payload_bytes ?? 0} byte
                      {selectedLog.mutation.target?.type
                        ? ` · ${selectedLog.mutation.target.type}`
                        : ""}
                      {selectedLog.mutation.target?.id
                        ? ` #${selectedLog.mutation.target.id}`
                        : ""}
                    </small>
                    {Object.entries(selectedLog.mutation.changes || {}).map(
                      ([field, change]) => (
                        <div className="audit-mutation-change" key={field}>
                          <b>{field}</b>
                          <span>
                            {change.value_logged === false
                              ? "Diubah · nilai sensitif tidak dicatat"
                              : `${formatAuditValue(change.from)} → ${formatAuditValue(change.to)}`}
                          </span>
                        </div>
                      ),
                    )}
                    {Object.keys(selectedLog.mutation.changes || {}).length ===
                      0 &&
                      Object.entries(
                        selectedLog.mutation.safe_values || {},
                      ).map(([field, value]) => (
                        <div className="audit-mutation-change" key={field}>
                          <b>{field}</b>
            ...
            <button
              className="btn btn-outline audit-trace-button"
              type="button"
              disabled={!selectedLog.trace_id || journeyLoading}
              onClick={loadJourney}
            >
              <i className="fa-solid fa-route"></i>
              {journeyLoading
                ? "Memuat perjalanan…"
                : "Lihat perjalanan request"}
            </button>

            {journeyError && (
              <div
                className="audit-feedback audit-feedback--error"
                role="alert"
              >
                {journeyError}
              </div>
            ...
            {journey.length > 0 && (
              <div className="audit-journey" aria-label="Perjalanan request">
                {journey.map((entry, index) => (
                  <div
                    className="audit-journey-step"
                    key={entry.id || `${entry.time}-${index}`}
                  >
                    <span
                      className={`audit-journey-marker ${entry.status === "failed" ? "is-failed" : ""}`}
                    >
                      {index + 1}
                    </span>
                    <div>
                      <div className="audit-journey-step-heading">
                        <strong>{getJourneyLabel(entry)}</strong>
                        <time>{formatLogTime(entry.time)}</time>
                      </div>
                      <span>
                        {entry.journey_outcome || entry.status}
                      ...
                      {entry.error_message && (
                        <small>{entry.error_message}</small>
                      )}
                    </div>
                  </div>
                ))}
